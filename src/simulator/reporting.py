...
STATUS_LABELS = {
    ...
    "error": "执行出错",
    "unavailable": "仿真环境不可用",
    "prepare_failed": "仿真环境准备失败",
    "import_failed": "程序导入失败",
}
...
STAGE_LABELS = {
    "connect": "连接 GX Simulator2",
    "cpu_reset": "复位仿真 CPU",
    "initial_write": "写入测试初始值",
    "initial_sample": "读取初始状态",
    "steps": "执行测试步骤",
    "complete": "测试完成",
    "preflight": "检查仿真环境",
    "mx_component": "检查 MX Component",
    "project": "检查 GX Works2 工程",
    "simulator": "启动 GX Simulator2",
}
...
def _assertion_issue(assertion: Mapping[str, Any]) -> str:
    address = _text(assertion.get("address")) or "未知软元件"
    ...
    if parts["operator"]:
        ...
        message = (
            f"{address} 实际为 {parts['actual']}，"
            f"期望{comparison} {parts['expected']}"
        )
        tolerance = parts["tolerance"]
        if tolerance not in {"0", "0.0", "0.00"}:
            message += f"（容差 {tolerance}）"
    else:
        message = f"{address} 未达到期望"
        if detail:
            message += f"：{detail}"
    wait_text = "，等待条件超时" if assertion.get("wait_for") else ""
...
def _device_name(address: Any, labels: Mapping[str, str]) -> str:
    ...
    return f"{label}（{normalized}）" if label else normalized or "未知软元件"
...
def _friendly_assertion_issue(
    ...
    if parts["operator"] == "eq" and _is_bit_device(address):
        ...
        if actual_on is not None and expected_on is not None:
            actual_text = "接通" if actual_on else "断开"
            expected_text = "接通" if expected_on else "断开"
            still = "仍" if actual_on != expected_on else ""
            message = (
                f"{device}实际{still}处于{actual_text}状态，"
                f"预期应{expected_text}"
            )
        else:
            message = f"{device}未达到预期状态"
    elif parts["operator"]:
        comparison = _OPERATOR_LABELS.get(parts["operator"], parts["operator"])
        message = (
            f"{device}实际值为 {parts['actual']}，"
            f"预期{comparison} {parts['expected']}"
        )
    else:
        message = f"{device}未达到预期状态"
...
def _friendly_set_action(
    ...
    if enabled is not None and "按钮" in label:
        return f"{'按下' if enabled else '松开'}{device}"
    if enabled is not None and "故障" in label:
        return f"{'触发' if enabled else '解除'}{device}"
    if enabled is not None and _is_bit_device(address):
        return f"将{device}{'接通' if enabled else '断开'}"
    return f"将{device}设为 {value}"
...
def _friendly_observation(
    ...
    if enabled is not None:
        return f"{device}{'已接通' if enabled else '已断开'}"
    if parts["actual"]:
        return f"{device}为 {parts['actual']}"
    return f"{device}状态正常"
...
def _invariant_issue(violation: Mapping[str, Any]) -> str:
    ...
    name = naturalize_identifier(raw_name, kind="运行约束") if raw_name else "运行约束"
    message = naturalize_display_text(violation.get("message")) or "运行状态违反了声明的约束"
...
def _case_timeline(
    ...
    for step in steps:
        ...
        if failed:
            ...
            if remainder > 0:
                summary += f"等 {len(step_assertions)} 项"
            actions.append("状态检查通过：" + summary)
            status = "passed"
        else:
            status = "action"
        if not actions:
...
def _case_report(
    ...
    if status != "passed" and not issues:
        missing = "测试未完成，但执行器没有返回更具体的错误信息。"
        issues.append(missing)
        friendly_issues.append(missing)
    technical_name = _text(case.get("name")) or "未命名测试"
    display_name = (
    ...
    return {
        ...
        "status_label": CASE_STATUS_LABELS.get(
            status,
            naturalize_identifier(status, kind="未知状态"),
        ),
        "stage": stage,
        "stage_label": STAGE_LABELS.get(
            stage,
            naturalize_identifier(stage, kind="未知阶段"),
        ),
        ...
        "summary": friendly_issues[0] if friendly_issues else "测试符合预期。",
...
def _recommendations(status: str, primary_reason: str) -> List[str]:
    ...
    if status == "failed":
        return [
            "这是程序行为或测试期望不一致，不是仿真连接故障。",
            "核对失败步骤对应的输入、输出和时序；确认期望无误后，可使用“故障调试”生成局部修改方案。",
        ]
    if "not_found" in lowered or "未找到" in primary_reason or "版本过旧" in primary_reason:
        return ["关闭旧版软件后使用当前修复版重新测试；程序会自动隔离旧版网关。"]
    if "timed out" in lowered or "timeout" in lowered or "超时" in primary_reason:
        return [
            "确认 GX Works2 已打开目标工程，GX Simulator2 能进入 RUN。",
            "若模拟器正在切换状态，请等待其稳定后重新测试。",
        ]
    if "mx component" in lowered:
        return ["确认已安装与网关位数匹配的 MX Component，并重新启动软件。"]
    if status == "import_failed":
        return ["先在 GX Works2 中确认目标工程已打开，再检查程序和注释 CSV 的导入提示。"]
    if status in {"error", "unavailable", "prepare_failed"}:
        return ["根据上方失败阶段和原始错误修复仿真环境后，再运行同一测试方案。"]
    return []
...
def render_simulator_report_text(report: Mapping[str, Any]) -> str:
    ...
    lines = [
        "仿真结果报告",
        f"状态：{_text(report.get('status_label'))}",
    ]
    suite_display_name = _text(report.get("suite_display_name")) or naturalize_identifier(
        report.get("suite_name"), kind="测试方案"
    )
    if suite_display_name:
        lines.append(
            "测试方案：" + suite_display_name
        )
    lines.extend(
        [
            f"结论：{_text(report.get('primary_reason'))}",
            (
                "统计："
                f"通过 {int(counts.get('passed') or 0)}，"
                f"失败 {int(counts.get('failed') or 0)}，"
                f"错误 {int(counts.get('error') or 0)}，"
                f"未执行 {int(report.get('not_executed_count') or 0)}"
            ),
        ]
    )
    ...
    for case_index, case in enumerate(problem_cases, start=1):
        ...
        lines.append(
            ...
            f"{preferred_display_name(case, kind='测试项目', index=case_index, descriptive_keys=('display_name', 'description', 'title', 'label'))}"
            f"{stage_text}"
        )
    ...
    if passed_cases:
        lines.extend(["", f"其余 {len(passed_cases)} 项测试通过。"])
    ...
    if _text(report.get("evidence_path")):
        lines.extend(["", "运行证据：已保存，可在结果窗口中打开。"])
    if cases:
        lines.extend(["", "执行详情："])
